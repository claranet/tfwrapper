...
def test_load_wrapper_config_old_map(tmp_working_dir_regional, default_args):  # noqa: D103
    ...
    paths["state_conf"].write_text(
        textwrap.dedent(
            ...
            azure:
                name: azure_backend
                general:
                    subscription_uid: 'f6eaa78a-3adf-4818-b0ea-87fb07acf477'
    ...
    assert wrapper_config["state"]["aws"]["state_region"] == "eu-west-1"
    assert wrapper_config["state"]["aws"]["state_profile"] == "terraform-states-profile"
    assert wrapper_config["state"]["azure_backend"]["state_subscription"] == "f6eaa78a-3adf-4818-b0ea-87fb07acf477"
